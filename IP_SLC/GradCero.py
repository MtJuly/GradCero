# ========================
# Run streamlit 
# In terminal, type --> streamlit run GradCero.py
# ========================
...
DB_PATH = os.path.join(BASE_DIR, "studentdb.xlsx")
FACEWARN_PATH = os.path.join(BASE_DIR, "facewarn.png")
FACECROSS_PATH = os.path.join(BASE_DIR, "facecross.png")

# ================================
# QR Generator - All Students
# ================================
def generate_qr():
    ...
    for _, row in df.iterrows():
        ...
        with open(qr_path, "rb") as f:
            msg.add_attachment(f.read(), maintype="image", subtype="png", filename=f"{student_id}.png")
...
# ================================
# QR Generator - Individual Student
# ================================
...
# Face Recognition
# ========================
def face_match_with_qr(proper_name, image_path, cloud_env=False):
    TOLERANCE = 0.50
    ...
    if best_box:
        ...
        if encodings:
            ...
            if is_match:
                ...
                announce_name(display_name, cloud_env=cloud_env)
                matched = True
            else:
...
# ========================
def announce_name(student_name, cloud_env=False):
    if cloud_env:
        try:
            from gtts import gTTS
            from io import BytesIO
            ...
            autoplay_audio = f"<audio autoplay controls><source src='data:audio/mp3;base64,{b64}' type='audio/mp3'></audio>"
            st.markdown(autoplay_audio, unsafe_allow_html=True)
            mp3_buffer.seek(0)
            st.audio(mp3_buffer, format="audio/mp3")
        except:
            st.info(f"Audio announcement skipped for {student_name}")
    else:
        try:
            import pyttsx3
            engine = pyttsx3.init()
            engine.setProperty('rate',160)
            engine.say(f"Now presenting, {student_name}")
            engine.runAndWait()
        except:
            st.info(f"Audio announcement skipped for {student_name}")
...
with st.sidebar:
    st.markdown("""
        <div style="background-color:#686868;color:white;font-weight:bold;padding:20px;
                    border-radius:8px 8px 0 0;text-align:center;font-size:20px;">
        System's Menu
        <div style="border-bottom:2px solid #444;width:100%;margin:20px auto -10px auto;"></div>
    ...
    menu = option_menu(menu_title=None,
                       ...
                       styles={
                           "container":{"padding":"5px","background-color":"#686868","border-radius":"0 0 8px 8px"},
                           "icon":{"color":"#E0E0E0","font-size":"20px"},
                           "nav-link":{"font-size":"16px","text-align":"left","margin":"5px 0","padding":"10px 20px",
                                       "border-radius":"8px","color":"#BCC3C7","font-weight":"bold"},
                           "nav-link-selected":{"background-color":"#2C2C2C","color":"white"}
                       })
...
def show_success_result(student_id, name, course, image_path):
    ...
    if remaining_time > 0:
        ...
        st.markdown(f"<p style='text-align:center'><b>Course:</b> {course}</p>", unsafe_allow_html=True)
        ...
        # Afterwards, clear up all the session to avoid redundant result for the next student scan
        st.session_state["result_show_time"] = None
...
if menu == "Dashboard (Scan & Verification)":
    ...
    # Session state, avoid redundant result for different person whenever being rerun
    ...
    with col1:
        ...
        # After click Start button, prompt out the QR Scan camera
...
# ================================================
    ...
    if os.path.exists("studentdb.xlsx"):
        ...
        col1, col2, col3, col4, col5, col6, col7 = st.columns([0.5, 1, 1.5, 0.8, 2, 2, 1])
        ...
        headers = ["No", "Student ID", "Name", "Course", "Image", "Email", "QR Generator"]
        ...
        for i, (idx, row) in enumerate(df.iterrows(), 1):
            ...
            col4.markdown(f"<div style='text-align:center; font-weight:bold;'>{row['Course']}</div>", unsafe_allow_html=True)

            key = f"img_{i}"
            if key not in st.session_state["img_toggle"]:
                st.session_state["img_toggle"][key] = False

            # Using button to toggle the img to be show or hide
            with col5:
                key = f"img_toggle_{i}"
            ...
            col6.markdown(f"<div style='text-align:center; font-weight:bold;'>{row['Email']}</div>", unsafe_allow_html=True)
